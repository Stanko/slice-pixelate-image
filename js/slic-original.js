export default class SLIC {
  ...
  showContours(ctx) {
    ...
    let istaken = Array.from({ length: this.height }).map(
      (linearray) => (linearray = Array.from({ length: this.width }).map((item) => (item = false))),
    );
        ...
        for (let j = startj; j < startj + this.stride && j < this.height; j++) {
          for (let k = startk; k < startk + this.stride && k < this.width; k++) {
            resultImage[4 * (j * this.width + k)] =
              this.rgbImage[4 * (this.centers[centerpos].x * this.width + this.centers[centerpos].y)];
            resultImage[4 * (j * this.width + k) + 1] =
              this.rgbImage[4 * (this.centers[centerpos].x * this.width + this.centers[centerpos].y) + 1];
            resultImage[4 * (j * this.width + k) + 2] =
              this.rgbImage[4 * (this.centers[centerpos].x * this.width + this.centers[centerpos].y) + 2];
            resultImage[4 * (j * this.width + k) + 3] =
              this.rgbImage[4 * (this.centers[centerpos].x * this.width + this.centers[centerpos].y) + 3];
          }
        }
      }
    }
